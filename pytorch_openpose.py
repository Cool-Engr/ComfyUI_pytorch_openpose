...
openpose_models_path = os.path.join(comfy_path, "custom_nodes/ComfyUI_pytorch_openpose/model/")
...
def generate_pose(img, body_estimation, hand_estimation):
    ...
    for i in range(len(candidate)):
        xs += candidate[i][0]
        ys += candidate[i][1]
    centroid = f'X:{int(xs/len(candidate))}\nY:{int(ys/len(candidate))}'
    ...
    canvas_black = np.zeros_like(oriImg)  
...
def process_one(photo, device='cuda:0'):
    body_estimation = Body(os.path.join(openpose_models_path, 'body_pose_model.pth'))
    hand_estimation = Hand(os.path.join(openpose_models_path, 'hand_pose_model.pth'))
    op_img, centroid_coords = generate_pose(photo, body_estimation, hand_estimation)
    return op_img, centroid_coords
